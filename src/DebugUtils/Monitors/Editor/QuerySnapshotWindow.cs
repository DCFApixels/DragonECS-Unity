...
namespace DCFApixels.DragonECS.Unity.Editors
{
	internal class QuerySnapshotWindow : EditorWindow
	{
		...
		private void OnGUI()
		{
			if (_world.IsNullOrDetroyed()) { _world = null; }
			if (_world == null)
			{
				Close();
				return;
			...
			{
				RawEntLong rawEntity = (RawEntLong)_list[i];
				bool isAlive = _world.IsAlive(rawEntity.id, rawEntity.gen);
				bool selected = selectedEntity == rawEntity.id && isAlive;
				...
				if (visible)
				{
					...
					if (i % 2 == 0)
					{
						EditorGUI.DrawRect(lineRect, new Color(0, 0, 0, 0.08f));
					}
					...
					var monitor = _world.Get<DragonGUI.EntityLinksComponent>().GetMonitorLink(rawEntity.id);
					var metaColor = Color.clear;
					var metaName = "";
					if (monitor)
					{
						metaColor = monitor.Color;
						metaName = monitor.name;
					}
					if (selected)
					{
					...
					if (isAlive)
					{
						var color = metaColor != Color.clear ? metaColor : _isAliveColor;
						DragonGUI.DrawRect(statusR, color);
					}



					var (labelR, infoR) = lineRect.HorizontalSliceLeft(160f);
					infoR.width = Mathf.Min(infoR.width, 100f);
					var (lR, rR) = infoR.HorizontalSliceLerp(0.5f);
					GUI.Label(labelR, metaName, GUI.skin.label);



					EditorGUI.IntField(lR, rawEntity.id, GUI.skin.label);
					EditorGUI.IntField(rR, rawEntity.gen, GUI.skin.label);

					if (isClick && isAlive)
					{
						selectEvent.Type = SelectEventType.Click;
						selectEvent.EntityID = rawEntity.id;
						selectEvent.Index = i;
					}
				}
		...
		{
			var monitor = _world.Get<DragonGUI.EntityLinksComponent>().GetMonitorLink(entityID);
